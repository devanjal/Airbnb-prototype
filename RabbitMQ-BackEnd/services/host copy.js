...
function becomehost_step1(msg, callback) {
    ...
    connectionpool.getConnection(function (err, connection) {
        ...
        if (msg.update == false) {
            ...
            connection.query(
                'UPDATE properties SET quantity = ?, address = ?, city = ?,state = ?,country = ?,zipcode = ? Where propertyid = ?',
                [msg.quantity, msg.address, msg.city, msg.state, msg.country, msg.zipcode, msg.propertyid],
                function (err, result) {
                    if (err) {
                        res.code = 401;
                        res.value = "Step1 failed";
